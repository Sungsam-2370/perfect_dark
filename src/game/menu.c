...
void menuProcessInput(void)
{
	...
	if (g_Menus[g_MpPlayerNum].curdialog) {
		...
		for (i = 0; i < numcontpads; i++) {
			...
			// D-pad as an extra digital way to navigate menus, same as the
			// C-buttons above. Uses dedicated CONT_0040/0080/0100/0200 bits
			// (bound to the physical D-pad in input.c) instead of U/D/L/R_JPAD,
			// because CK_DPAD_U/D/L/R can be freely remapped to other gameplay
			// actions and would no longer reflect the physical D-pad position.
			if (buttons & 0xfff00000) {
				sysLogPrintf(LOG_NOTE, "menu dpad debug: buttons=0x%08x buttonsnow=0x%08x", buttons, buttonsnow);
			}

			if (buttons & CONT_0040) {
				yhelddir = -1;
			}

			if (buttonsnow & CONT_0040) {
				ytapdir = -1;
			}

			if (buttons & CONT_0080) {
				yhelddir = 1;
			}

			if (buttonsnow & CONT_0080) {
				ytapdir = 1;
			}

			if (buttons & CONT_0100) {
				xhelddir = -1;
			}

			if (buttonsnow & CONT_0100) {
				xtapdir = -1;
			}

			if (buttons & CONT_0200) {
				xhelddir = 1;
			}

			if (buttonsnow & CONT_0200) {
				xtapdir = 1;
			}
