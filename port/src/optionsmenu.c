...
struct menuitem g_ExtendedBindKeyMenuItems[] = {
	...
	},
	{
		...
		0,
		MENUITEMFLAG_SELECTABLE_CENTRE | MENUITEMFLAG_LITERAL_TEXT,
		(uintptr_t)"ESC to cancel, DEL or gamepad - to remove binding\n",
		0,
		menuhandlerDoBind,
	},
	{ MENUITEMTYPE_END },
...
static MenuItemHandlerResult menuhandlerDoBind(s32 operation, struct menuitem *item, union handlerdata *data)
{
	...
	if (key && key != VK_ESCAPE) {
		// treat the "-" / Back button on the active player's gamepad the same as
		// the keyboard DEL key, so binds can be cleared without a keyboard
		const s32 padBackKey = VK_JOY_BEGIN + g_ExtMenuPlayer * INPUT_MAX_CONTROLLER_BUTTONS + (VK_JOY1_BACK - VK_JOY1_BEGIN);
		const s32 isDelete = (key == VK_DELETE) || (key == padBackKey);
		inputKeyBind(g_ExtMenuPlayer, g_BindContKey, g_BindIndex, (isDelete ? 0 : key));
		// flush the new binding to disk immediately instead of relying on a
		// clean process exit (cleanup()/atexit never runs on Switch when the
		// game is closed via HOME/hbmenu, so binds were never actually saved)
		inputSaveBinds();
