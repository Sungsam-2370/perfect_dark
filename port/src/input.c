...
{
	// sentinel for "no bind" in the *joypad* tables below. We can't use 0 for
	// this like the keyboard tables do, because SDL_CONTROLLER_BUTTON_A is
	// itself 0 -- treating it as "empty" was silently unbinding CK_CANCEL/CK_B.
	#define JB_NONE ((u32)-1)
	...
	static const u32 pcjoybinds[][3] = {
		{ CK_A,      SDL_CONTROLLER_BUTTON_B,                JB_NONE                                      }, // swapped from _A (Switch A/B layout fix)
		{ CK_X,      SDL_CONTROLLER_BUTTON_Y,                JB_NONE                                      }, // swapped from _X (Switch X/Y layout fix)
		{ CK_Y,      SDL_CONTROLLER_BUTTON_RIGHTSHOULDER,    SDL_CONTROLLER_BUTTON_DPAD_RIGHT             }, // swapped from _Y (Switch X/Y layout fix)
		{ CK_DPAD_L, SDL_CONTROLLER_BUTTON_LEFTSHOULDER,     SDL_CONTROLLER_BUTTON_DPAD_LEFT              }, // swapped from _B (Switch A/B layout fix)
		{ CK_DPAD_D, SDL_CONTROLLER_BUTTON_DPAD_UP,          JB_NONE                                      },
		{ CK_LTRIG,  SDL_CONTROLLER_BUTTON_X,                SDL_CONTROLLER_BUTTON_RIGHTSTICK             },
		{ CK_RTRIG,  VK_JOY1_LTRIG - VK_JOY1_BEGIN,          JB_NONE                                      },
		{ CK_ZTRIG,  VK_JOY1_RTRIG - VK_JOY1_BEGIN,          JB_NONE                                      },
		{ CK_START,  SDL_CONTROLLER_BUTTON_START,            JB_NONE                                      },
		{ CK_C_D,    JB_NONE,                                JB_NONE                                      },
		{ CK_C_U,    JB_NONE,                                JB_NONE                                      },
		{ CK_C_R,    JB_NONE,                                JB_NONE                                      },
		{ CK_C_L,    JB_NONE,                                JB_NONE                                      },
		{ CK_ACCEPT, SDL_CONTROLLER_BUTTON_B,                JB_NONE                                      }, // swapped from _A (Switch A/B layout fix)
		{ CK_CANCEL, SDL_CONTROLLER_BUTTON_A,                JB_NONE                                      }, // swapped from _B (Switch A/B layout fix)
		{ CK_B,      SDL_CONTROLLER_BUTTON_A,                JB_NONE                                      }, // same physical button as CK_CANCEL; some menus check B_BUTTON instead of BUTTON_UI_CANCEL
		{ CK_8000,   SDL_CONTROLLER_BUTTON_LEFTSTICK,        SDL_CONTROLLER_BUTTON_DPAD_DOWN              },
		// dedicated menu-navigation bits, always tied to the physical D-pad
		// regardless of how CK_DPAD_U/D/L/R are remapped for gameplay above
		{ CK_0040,   SDL_CONTROLLER_BUTTON_DPAD_UP,          JB_NONE                                      },
		{ CK_0080,   SDL_CONTROLLER_BUTTON_DPAD_DOWN,        JB_NONE                                      },
		{ CK_0100,   SDL_CONTROLLER_BUTTON_DPAD_LEFT,        JB_NONE                                      },
		{ CK_0200,   SDL_CONTROLLER_BUTTON_DPAD_RIGHT,       JB_NONE                                      },
	};
	...
	static const u32 n64joybinds[][3] = {
		{ CK_A,      SDL_CONTROLLER_BUTTON_B,              JB_NONE }, // swapped from _A (Switch A/B layout fix)
		{ CK_B,      SDL_CONTROLLER_BUTTON_A,              JB_NONE }, // swapped from _B (Switch A/B layout fix)
		{ CK_LTRIG,  SDL_CONTROLLER_BUTTON_LEFTSHOULDER,   JB_NONE },
		{ CK_RTRIG,  SDL_CONTROLLER_BUTTON_RIGHTSHOULDER,  JB_NONE },
		{ CK_ZTRIG,  VK_JOY1_RTRIG - VK_JOY1_BEGIN,        JB_NONE },
		{ CK_START,  SDL_CONTROLLER_BUTTON_START,          JB_NONE },
		{ CK_DPAD_D, SDL_CONTROLLER_BUTTON_DPAD_DOWN,      JB_NONE },
		{ CK_DPAD_U, SDL_CONTROLLER_BUTTON_DPAD_UP,        JB_NONE },
		{ CK_DPAD_L, SDL_CONTROLLER_BUTTON_DPAD_LEFT,      JB_NONE },
		{ CK_DPAD_R, SDL_CONTROLLER_BUTTON_DPAD_RIGHT,     JB_NONE },
		// dedicated menu-navigation bits, always tied to the physical D-pad
		// (redundant with CK_DPAD_U/D/L/R above in this table since N64 mode
		// doesn't remap them, but kept for consistency with the PC table)
		{ CK_0040,   SDL_CONTROLLER_BUTTON_DPAD_UP,        JB_NONE },
		{ CK_0080,   SDL_CONTROLLER_BUTTON_DPAD_DOWN,      JB_NONE },
		{ CK_0100,   SDL_CONTROLLER_BUTTON_DPAD_LEFT,      JB_NONE },
		{ CK_0200,   SDL_CONTROLLER_BUTTON_DPAD_RIGHT,     JB_NONE },
	...
	for (u32 i = 0; i < numjoybinds; ++i) {
		for (s32 j = 1; j < 3; ++j) {
			if (joybinds[i][j] != JB_NONE) {
				inputKeyBind(cidx, joybinds[i][0], j - 1, VK_JOY_BEGIN + cidx * INPUT_MAX_CONTROLLER_BUTTONS + joybinds[i][j]);
			}
		}
	}
